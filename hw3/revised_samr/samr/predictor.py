...
class PhraseSentimentPredictor:
    ...
    def predict2(self, phrases):
        """
        based on predict, but use word2vec dictionary to predict
        """
        Z = self.pipeline.transform(phrases)
        labels = self.classifier.predict(Z)
        if self.duplicates:
        ...
        fdict=open(os.path.join(DATA_PATH,"test-dict.txt"))
        dict=[]
        l=[]
        i=0
        for lines in fdict:
